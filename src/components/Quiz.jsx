  ...
  return (
    ...
    <header className='flex gap-2 px-2 py-6'>
        {Array.from(Array(questions.length).keys()).map((i)=>(
            <div key={i} className={`h-3 bg-red-600 w-full rounded-md bg-opacity-40 ${i <= currentQuestion ? "bg-opacity-100" : ""}`}>
                </div>
        ))}
    </header>
    <section className='h-screen w-full flex items-center justify-center'>
        <div className='bg-gray-100 shadow-md w-[500px] py-10 px-6 text-center rounded-md'>
            ...
            <div className='Passed'>
                ...
                <button className='bg-green-600 px-8 py-2 rounded-full text-white '>Print Certificate</button>
            </div>) : (
            <div className='Failed'>
            <h3 className='text-xl font-semibold'>Failed</h3>
                <h2 className='text-6xl mb-5 font-extrabold text-red-700'>{correctCounter / questions.length * 20}%</h2>
                <p className='mb-5'>You Got {correctCounter}/{questions.length} score</p>
                <button className='bg-red-600 px-8 py-2 rounded-full text-white' onClick={handleRetake}>Retake</button>
            </div>)}
            ...
            <button  className='bg-red-600 px-8 py-2 rounded-full text-white ' onClick={handleNextQuestion}>Next Question</button>
            </div>)}
